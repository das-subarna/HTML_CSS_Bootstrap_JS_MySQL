...
function setUpCard(book) {
    ...
    cardImageTop.setAttribute('title', 'Know More')
    ...
    h5.setAttribute('onclick', KM)
    h5.setAttribute('title', 'Know More')
    ...
    p.setAttribute('onclick', KM)
    p.setAttribute('title', 'Know More')
...
function minus(num) {
    ...
    if (Number(x.innerText) > 1) {
        ...
        alert("Single item exists! Click 'Delete' to remove.");
...
function knowMore(num) {
    ...
    /*document.getElementById("knowMoreFooter").innerHTML =
        `<button type="button" class="btn btn-info btn-sm float-right" id="a2c`
        + book[i].id +
        `" onclick="add2cart(`
        + book[i].id +
...
function billingTotal(){
    document.getElementById("total_billing").innerText = sum;
}
...
function clear1(){
    ...
    alert('Payment Gateway Not Linked. Page will be reset now.\n Thank you!\n\n -Subarna Das')
}